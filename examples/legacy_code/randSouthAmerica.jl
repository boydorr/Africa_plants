...
function ecopopulate!(ml::GridLandscape, spplist::SpeciesList,
  ...
  for i in eachindex(spplist.abun)
      if spplist.native[i] &  all(.!isnan.(probabilities[:, i]./sum(probabilities[:, i])))
...
JLD.save("SA_rand_1.jld", "diver", EcoSISTEM.SavedLandscape(output.abundances))
...
#ws4
for i in 1:10
    output = buildEco("SouthAmerica/SA_rand_1.jld", false, "SouthAmerica/rand/run/", "SA_$i")
    JLD.save("SouthAmerica/SA_rand_full_$i.jld", "diver", output)

    output = buildEco("SouthAmerica/SA_rand_1.jld", true, "SouthAmerica/rand/neutral/", "SA_$i")
    JLD.save("SouthAmerica/SA_rand_neutral_$i.jld", "diver", output)
    print("$i \n")
end
